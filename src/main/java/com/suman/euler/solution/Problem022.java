...
public class Problem022 implements EulerProblem {
	private final String[] names = { "MARY", "PATRICIA", "LINDA", "BARBARA",
			"ELIZABETH", "JENNIFER", "MARIA", "SUSAN", "MARGARET", "DOROTHY",
			"LISA", "NANCY", "KAREN", "BETTY", "HELEN", "SANDRA", "DONNA",
			"CAROL", "RUTH", "SHARON", "MICHELLE", "LAURA", "SARAH",
			"KIMBERLY", "DEBORAH", "JESSICA", "SHIRLEY", "CYNTHIA", "ANGELA",
	...
	public Object executeSoultion() {
		...
		for (int index = 0; index < nameListLength; index++) {
			String name = nameList.get(index);
			totalScore += getScore(name, index + 1);
		}
		return totalScore;
	}

	private long getScore(String name, int position) {
		int nameLength = name.length();
		long sumOfLetterNo = 0L;
		for (int index = 0; index < nameLength; index++) {
			sumOfLetterNo += getLatterNo(name.charAt(index));
		}
		return sumOfLetterNo * position;
	}

	private long getLatterNo(char letter) {
		long letterNo = 0L;
		switch (letter) {
		case 'A':
			letterNo = 1;
