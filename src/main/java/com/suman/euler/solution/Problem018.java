...
public class Problem018 implements EulerProblem {
	private final int[][] TRIANGLE = { { 75 }, { 95, 64 }, { 17, 47, 82 },
	...
	public Object executeSoultion() {
		long maxTotal = TRIANGLE[0][0] + getNextMaxValue(0, 0);
		return maxTotal;
	}

	private long getNextMaxValue(int row, int col) {
		++row;
		long maxVal = 0;
		if (row < TRIANGLE.length) {
			long valueOne = TRIANGLE[row][col] + getNextMaxValue(row, col);
			++col;
			long valueTwo = TRIANGLE[row][col] + getNextMaxValue(row, col);
			maxVal = valueOne >= valueTwo ? valueOne : valueTwo;
		}
		return maxVal;
	}
}
